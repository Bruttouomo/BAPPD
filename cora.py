...
class CoraSetup(DatasetSetup):
    def __init__(self):
        super().__init__()
        self.num_classes = 7  # Cora数据集有7个类别
        self.size_bottom_out = 7  # 底部模型输出的大小为类别数

    def set_datasets_for_ssl(self, file_path, n_labeled, party_num=None):
        # 加载Cora数据集
        dataset = Planetoid(root=file_path, name='Cora')
        ...
        # 进行标记数据和未标记数据的分割
        # 注意：在Cora数据集上，train_mask已经标识了哪些数据是用于训练的
    ...
    def get_transforms(self):
        # Cora数据集的特征已经准备好了，所以不需要额外的transform
        return None
